...
test("model + effort reserve room independently for whichever piece is present (not 0 when model is blank)", () => {
  ...
  // the effort piece reads effortWord: the lane's level, or the bare kind for a live Codex lane with none picked (2026-09-16)
  assert.match(SRC, /const effortWord = \(s\) => s\.effort \|\| \(s\.live && s\.backend === 'codex' \? 'effort' : ''\);/);
  assert.match(SRC, /const effortPieceW = \(s\) => \(effortWord\(s\) \? this\.ctxWidth\(effortWord\(s\)\) \+ caretW : 0\);/);
...
test("the effort is left-justified to a FIXED column (the user 2026-07-03): same x for every lane", () => {
  ...
  assert.match(SRC, /if \(effortWord\(s\)\) drawPiece\('effort', effortWord\(s\), effortColX\);/);
...
test("the picker draws when EITHER model or effort is present, each piece guarded independently", () => {
  assert.match(SRC, /if \(s\.model \|\| effortWord\(s\)\) \{/);   // effortWord: a live Codex lane's picker stands before a pick (2026-09-16)
