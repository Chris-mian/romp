...
class ServedSettingsTabs(unittest.TestCase):
    ...
    def test_the_pills_read_general_chat_feed_sessions_automation_task_tracking_debug_in_that_order(self):
        ...
        self.assertEqual(g["heads"]["chat"], ["Display", "Comments", "Thinking", "Tab strip", "Tab widgets", "Status line"], "Transcript is Display; the text scheme and the strip row joined it; Thinking creates, so it is Chat's; the Status line section follows Tab widgets (T409)" + table)
