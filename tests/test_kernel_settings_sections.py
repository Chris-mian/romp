...
class SettingsSectionsTest(unittest.TestCase):
    ...
    def test_each_setting_sits_under_the_right_section(self):
        h = _gear_src()
        where = {
            "general": ["rs-billing", "rs-login-btn", "rs-panes-sec", "rs-pane-timeline", "rs-pane-fleet", "rs-pane-feed", "rs-filesctl", "rs-artctl", "rs-panedock", "rs-theme", "rs-cmap", "rs-pal", "rs-fileedit", "rs-conserve", "rs-updates"],
        ...
        self.assertEqual(pn.count('<label class="rs-row rs-panes-row">'), 6)   # …and the Artifacts row (plans/artifacts-pane.md, 2026-09-19)   # Sessions, Outline, Feed, the Files row since the T404 tidy, and the Pane docking switch (plans/pane-docking.md phase two: a Panes row, so the off-dashboard hide takes it)
        ...
        ge = panes["general"]
        self.assertTrue(ge.index(">Account<") < ge.index("id=rs-login-btn") < ge.index("id=rs-panes-sec") < ge.index("id=rs-pane-feed") < ge.index("id=rs-filesctl") < ge.index("id=rs-artctl")
