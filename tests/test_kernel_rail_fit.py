...
gear, always visible). The usage bars DISAPPEAR first when room runs out (fitRail → .rail-tight); only if even
the toggles don't fit do they scroll (hidden scrollbar). So the gear/refresh never collide with the labels."""
import os
import unittest
...
class RailFit(unittest.TestCase):
    ...
    def test_scroll_group_styling_and_hidden_scrollbar(self):
        ...
        self.assertIn(".rail-acts{flex:0 0 auto;display:flex;flex-direction:column;gap:6px;margin-top:auto}", land)
    ...
    def test_usage_bars_hide_when_tight(self):
        self.assertIn(".rail-scroll.rail-tight #rail-usage{display:none}", km._landing())
        self.assertNotIn("#rail-refresh{margin-top:auto}", km._landing(), "the old per-action pin is gone")

    def test_fitRail_drops_usage_first_on_overflow_and_re_runs(self):
        js = km._LANDING_USAGE_JS
        self.assertIn("function fitRail()", js)
        self.assertIn("var sc=el.parentNode;", js)
        self.assertIn("sc.classList.remove('rail-tight')", js)
        self.assertIn("if(sc.scrollHeight>sc.clientHeight+1)sc.classList.add('rail-tight')", js)
