...
class AnchorlessCreate(unittest.TestCase):
    ...
    its transcript, so the newest turn may not be in the DOM at all. The kernel takes such a create and
    forks at the tip, which is what keeps the affordance from disappearing on a long conversation."""

    def test_the_handler_accepts_a_create_with_src_and_no_uuid(self):
        """The create door opens on a src label alone, with no anchor uuid."""
    ...
    def test_an_empty_anchor_forks_at_the_tip_instead_of_refusing(self):
        """An empty anchor takes the tip-fork branch: no cut record, the create moment as its time."""
        src = open(os.path.join(BIN, "romp-kernel"), encoding="utf-8").read()
        self.assertIn('cut, cut_t = "", int(now)', src)
        self.assertIn('"anchorUuid": str(anchor_uuid or "")', src)
