...
def test_connect_push_does_not_advance_the_shared_baseline():
    ...
    tests 18 to 26 drive it), so the regex below stops at the ledger line and leaves the branch to them. The write and
    the clear of the seed's raced mark are one step under _chat_baseline_lock (2026-09-19), so the regex admits that
    `with` line between the gate and the two writes.
    """
...
def test_ready_branch_is_wired_to_the_reset():
    ...
    assert i > 0
    body = src[i:i + 3000]   # the arm grew past 1600 on 2026-09-11 (the focused-session send, the connect-time reads and the skeleton client's re-arm landing together): the connect push sits near 2400 now; the window follows tests/test_chat_skeleton_reconnect.py's
