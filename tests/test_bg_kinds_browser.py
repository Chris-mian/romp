...
if (keys.entered) {
  ...
  const f0 = cfg.frame;
  const pushed = async (frame, until) => { await page.evaluate((x) => window.postMessage(x, "*"), frame); await page.waitForFunction(until, null, { timeout: 6000 }).catch(() => {}); return bgActive(); };
  keys.statusPush = await pushed({ type: "status", id: f0.id, status: { ...f0.status, awaitingWhy: "waiting on 1 background agent: the docs build" } }, () => ((document.querySelector("#bg-tasks .bg-fold-label") || {}).textContent || "").includes("docs build") || true);
  keys.framePush = await pushed(f0, () => !!document.querySelector("#bg-tasks .bg-fold-head"));
...
class ServedBgKinds(unittest.TestCase):
    ...
    def test_the_header_keeps_the_keyboard_through_its_own_toggle_and_every_push_but_not_a_mouse_clicks_focus(self):
        ...
        self.assertTrue(k["statusPush"]["head"], "a push changing an awaited field keeps the focus on the header: %r" % k["statusPush"])
        self.assertTrue(k["framePush"]["head"], "and so does a full session frame: %r" % k["framePush"])
