...
- `set_working(text)` — publish what you're working on (files/surface) so peers see it in `list_agents`; pass empty text to clear when done (romp also auto-clears it once your work is done and the session goes idle, so a finished session stops advertising a stale claim).
...
- `recall_message(to, id?)` — unsend a message the recipient hasn't read yet (still queued) — e.g. when the ask went moot. Pass `to` to recall your unread message(s) to them, or add `id` (from `check_sent`) to cancel just one. Only your own, still-unread messages.

Addressing is **live-only**: you can only message sessions that are currently live (see `list_agents`). A name that isn't a live session errors — there's no parking mail for, or reviving, dead sessions.

The same thing from the shell, and for the human: `romp --mail send <name> "<text>"`, `romp --mail inbox`, `romp --mail agents`, `romp --mail working "<note>"`, `romp --mail sent`, `romp --mail recall <name> [id]`.
...
**Coordinate before parallel edits — by READING state, not by waking peers.** When delegating to or working alongside a peer, declare what you own ("I own files A/B — stay off them") in the first line. Before editing in a shared repo, run `list_agents` to see what others have published (their working-on note + branch — file overlap is only a real collision on the *same* branch), and publish your own with `set_working`. **Resolve ownership by reading that state — never send a message to ask "do you still own this?"** Discount a stale claim instead of acting on it: a note flagged `(idle now — claim may be stale)` is from a session that has finished its turn, and a peer with NO note isn't actively holding files (romp auto-clears the note when a session's work is done). Treat a file as owned only when the peer is on your branch AND actively working it *now* — and never wake an idle session just to disambiguate ownership; that's a false interrupt, the exact thing romp exists to avoid. Use `check_sent` to see if a message was read instead of asking "did you get it?". Confirm substantive coordination (a delegation plan, an ownership boundary) — but never just to acknowledge.
