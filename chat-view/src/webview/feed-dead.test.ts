...
// shared agent element, so it must TOGGLE the class per target and CLEAR it on the always-live blocked card.
...
test("the modal's agent name is struck through for a dead session, cleared for a (live) blocked card", () => {
  ...
  // …and the blocked card explicitly clears it (a blocked session is always live)
  assert.match(FEED, /agent\.classList\.remove\("dead"\)/);
});
