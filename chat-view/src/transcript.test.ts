...
import { parse, newIncParser, feed, buildParsed, ChatEvent, stripAnsi, cleanCommandLine } from "./transcript";

// ---- local-command meta cleanup (the user 2026-06-14): caveat/ANSI/compact handling ----
// Synthetic inputs only (no real session data) per the repo privacy rule.
test("stripAnsi removes leaked CSI/SGR escape codes", () => {
  assert.equal(stripAnsi("\x1b[2mdim\x1b[22m text"), "dim text");
});

test("cleanCommandLine: a /compact command line reports compact", () => {
  assert.deepEqual(cleanCommandLine("<command-name>/compact</command-name><command-message>compact</command-message>"), { compact: true, text: "" });
});

test("cleanCommandLine: the compaction stdout (Compacted (ctrl+o…) reports compact", () => {
  assert.equal(cleanCommandLine("\x1b[2mCompacted (ctrl+o to see full summary)\x1b[22m").compact, true);
});

test("cleanCommandLine: a non-compact slash command becomes '/name args', not tag soup", () => {
  assert.deepEqual(
    cleanCommandLine("<command-name>color</command-name><command-message>color</command-message><command-args>blue</command-args>"),
    { compact: false, text: "/color blue" });
});

test("cleanCommandLine: caveat + wrappers + hook-completion lines are stripped, real output kept", () => {
  const raw = "Caveat: The messages below are generated by the user while running local commands. DO NOT respond to these messages as if they are coming from the user. "
    + "<local-command-stdout>hello from the script\nPostStop [/path/hook.sh] completed successfully</local-command-stdout>";
  const { compact, text } = cleanCommandLine(raw);
  assert.equal(compact, false);
  assert.equal(text, "hello from the script");
});
...
test("compaction stitch: logicalParentUuid keeps pre-compaction history", () => {
  ...
  const out = parse(raw);
  const texts = out.events.map((e: any) => e.md ?? "");
  assert.ok(texts.includes("early work"), "pre-compaction history must survive");
  assert.ok(texts.includes("post-compaction reply"));
  // the summary itself collapses to a clean compact MARKER, not its wall of text (the user 2026-06-14)
  assert.ok(out.events.some((e: any) => e.kind === "compact"), "a compact marker is emitted");
  assert.ok(!texts.includes("summary of earlier conversation"), "the summary text is not rendered");
});
