...
export type ChatEvent =
  ...
  // A context compaction — collapsed from the raw /compact command + its ANSI/hook stdout
  // into one clean marker (the user 2026-06-14).
...
export function buildParsed(p: IncParser): ParsedTranscript {
  ...
  for (const o of path) {
    ...
    if (o.type === "user") {
      // The auto-generated compaction SUMMARY (a synthetic user line) → one clean marker, not its
      // wall of summary text (the user 2026-06-14). Dedup so the summary + the stdout echo coalesce.
      if (o.isCompactSummary === true) {
        if (events[events.length - 1]?.kind !== "compact") events.push({ kind: "compact", ts, uuid });
        continue;
      }
      ...
      // Local-command meta: a slash command + its terminal output arrive as a NON-human user
      // line wrapping <command-*>/<local-command-stdout> tags, prefixed by a model-directed
      // "Caveat:" line. Strip the ANSI + caveat + wrappers; collapse a /compact to ONE clean
      // "Compacted" marker (the user 2026-06-14). Genuine typed messages never match CMD_META_RE.
      if (CMD_META_RE.test(raw)) {
        const { compact, text } = cleanCommandLine(raw);
        if (compact) {
          if (events[events.length - 1]?.kind !== "compact") events.push({ kind: "compact", ts, uuid });
          continue;
        }
...
// ---- local-command meta cleanup (the user 2026-06-14) ----
// A user line is local-command META — NOT the human talking — when it carries a slash-command
// wrapper, a local-command stdout block, or Claude Code's model-directed "Caveat:" preamble.
const CMD_META_RE = /<command-(?:name|message|args|contents)>|<local-command-(?:std(?:out|err)|caveat)>|Caveat: The messages below are generated by the user while running local commands|Compacted \(ctrl\+o/;
// CSI / SGR escape sequences (the "\x1b[2m" dim codes that leak into captured stdout).
const ANSI_RE = /\x1b\[[0-9;]*[A-Za-z]/g;
export function stripAnsi(s: string): string { return s.replace(ANSI_RE, ""); }

// Turn a local-command meta line into either a compaction marker, or its cleaned display text
// (ANSI stripped, the model-directed caveat + command wrappers + hook-completion noise removed).
// `text` may be "" → nothing worth rendering. A /compact (or its "Compacted (ctrl+o…" stdout)
// reports { compact: true }.
export function cleanCommandLine(raw: string): { compact: boolean; text: string } {
  const s = stripAnsi(raw);
  const nameM = s.match(/<command-name>\s*\/?([\w:-]+)/i);
  // a /compact invocation OR its "Compacted (ctrl+o…" stdout → one clean marker
  if ((nameM && nameM[1].toLowerCase() === "compact") || /Compacted \(ctrl\+o/i.test(s)) return { compact: true, text: "" };
  // a slash-command INVOCATION → "/name args" (not the concatenated tag soup)
  if (nameM) {
    const args = (s.match(/<command-args>([\s\S]*?)<\/command-args>/i)?.[1] || "").trim();
    return { compact: false, text: ("/" + nameM[1] + (args ? " " + args : "")).trim() };
  }
  // command OUTPUT → keep the real stdout, drop the model-directed caveat + hook-completion chatter
  const stdout = s.match(/<local-command-stdout>([\s\S]*?)<\/local-command-stdout>/i);
  const text = (stdout ? stdout[1] : s)
    .replace(/Caveat: The messages below are generated by the user while running local commands\.\s*DO NOT respond to these messages as if they are coming from the user\.\s*/g, "")
    .replace(/^\s*(?:Pre|Post)\w+ \[[^\]]*\] completed successfully\s*$/gim, "")
    .trim();
  return { compact: false, text };
}
