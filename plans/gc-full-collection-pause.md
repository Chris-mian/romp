...
- A BOUNDED BACKSTOP reclaim runs after a stated number of load fold-ins since the last reclaim (`backstopFoldins`, default 1000, about ten hours at the measured ~97 fold-ins an hour), so a cycle released on a path no note reaches is bounded by the next thousand loads, not the process lifetime. The backstop, not the note, is the correctness floor; the note only makes the rare cyclic case prompt.
