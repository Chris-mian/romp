...
The reclaim is therefore keyed on the OBSERVED cyclicity of ended sessions, plus a bounded backstop:

- WHEN to reclaim is MEASURED, not guessed. Three review rounds each guessed a session's cyclicity from a state flag (its `client`, its worker thread) and each found a new path the guess missed: a teardown exception raised after the connect loop cleared `self.client` leaves a session with client None that is still CYCLIC (a traceback frame reaches it), popped by `_on_session_gone`, that the client guard would never note. So the guess is replaced by observation: every session-end pop (the three `self.sessions.pop` in the SDK backend, and any future one) REGISTERS a `weakref.ref(session)` with its worker thread via `_note_ended`, deciding nothing; at the idle tick the controller judges each by what it observes. A ref that died went by reference counting (acyclic, no reclaim); a ref alive whose worker thread has finished is a surviving cycle (reclaim once for all such); a ref alive whose thread still runs is not garbage yet (a kill mid-turn, judged again next tick). This is exact on every path (the connect-raise, the teardown failure, the clean exit, the kill), costs one weakref per session end, and turns the kill and conserve-close pops from a wasted full pause into a check. The record cache, the parsed session trees and the materialized atoms are acyclic and register nothing; `recordCache.released` stays a `/perf` statistic. The Codex backend's four `_sessions.pop` sites are FAILED-SPAWN ROLLBACKS, not session ends, and register nothing; any cyclic release no ended ref reaches is caught by the backstop.
- A BOUNDED BACKSTOP reclaim runs after a stated number of load fold-ins since the last reclaim (`backstopFoldins`, default 1000, about ten hours at the measured ~97 fold-ins an hour), so a cycle released by an owner nobody registered is bounded by the next thousand loads, not the process lifetime. The backstop is the correctness floor; the ended-ref judgement makes the common cyclic case prompt.

The thread test rests on CPython's own threading. A running session and its worker `Thread` reference each other (the `Thread` holds the session as its `_target`), and `Thread.run` deletes `_target` in a `finally` when it returns, so a finished thread no longer holds its session. A registered ref still alive once its thread has finished is therefore held by a DIFFERENT surviving cycle (a traceback frame from a teardown raise, a stray reference), which is exactly what the reclaim must take. A session whose thread never STARTED reads as finished (`is_alive()` is False) and still holds the session: the `Thread` keeps its `_target` (bound to the session's `_run`), since `run`'s finally never fires for a thread that never ran. So the ref is alive on that cycle, and judging it a reclaim is correct precisely because ONLY the collector can take the cycle (refcounting never will), which is what the reclaim does. A ref a live ROOT keeps, not a cycle (a never-joined helper thread's `_target`, a strong reference), reads the same, alive with a finished worker, and is treated as a surviving cycle: the reclaim frees nothing, so it costs exactly ONE full-heap pause, is counted a `survivor` on `/perf` (the tick re-reads the owed refs after the collect and the re-freeze, which is harmless since a freeze collects nothing), and is dropped, never re-registered. The RELEASE is STAGED so the common case is cheap: a plain `collect` with the freeze in place first takes a released cycle allocated since the last freeze in under a millisecond, and only a ref that survives that drives the `unfreeze` and the frozen-heap walk; a release the cheap collect took whole counts as a load pass, so the backstop's bound does not stretch. Each full (unfrozen) release writes one stderr line naming the ended session; `lastReleaseSids` on `/perf` carries the first eight characters of the sids the LAST JUDGEMENT owed a reclaim for, cleared each judgement (so a tick that owed nothing clears it, and a cheap-collect release, judged `load`, shows them too).

The LOAD fold-in is unchanged: keyed on the record cache's insert counter, a cheap `collect` then `freeze` that walks only the unfrozen. Measured over a warm hour (plans method above, the live insert rate of about 780 an hour): ZERO reclaims when nothing cyclic is released, at ten times that rate still zero, and one reclaim per session that ends holding a surviving cycle (a rare connect-raise or teardown failure), never for the common acyclic exit. That is near-zero beside the roughly seventy organic full collections an hour the freeze removes.
