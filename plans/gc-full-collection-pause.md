...
- A cyclic owner NOTES its release, but ONLY when the cycle actually survives the exit. A second executed review (2026-09-22) found the exited backend session is usually acyclic: its worker Thread is the cycle, and CPython breaks a finished thread's cycle, so a session run to exit (its thread done, its `client` cleared) dies by reference counting; the cycle survives only on the connect-raised path, where a live `client` outlasts the exit. So the three `self.sessions.pop` sites note a release only when `session.client is not None` (via `_note_session_release`); the common exit notes nothing and pays no reclaim. The record cache, the parsed session trees and the materialized atoms are acyclic and note nothing; `recordCache.released` stays a `/perf` statistic. The Codex backend's session-end pops are self-healing by reference counting the same way and note nothing; a cyclic release either backend misses is caught by the backstop.
