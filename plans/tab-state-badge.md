...
The left STATUS DOT is `.tab-dot`, classed by `tabDotClass` (`tab-state.ts:133`): working is `.tab-dot` (gold `--st-working-bg`, `styles.css:603`), awaitingBg is `.tab-dot await` (green `--st-awaitbg-bg`, `:604`), opening is `.tab-dot opening` (`:615`), an unknown state is `.tab-dot unknown` (a grey ring, `:611`), COMPACTING is null (the compacting bar takes the slot, not a dot), and EVERYTHING ELSE, including retrying and blocked, is `.tab-dot none` (hidden, `:607`); the dot widget's "Grey dot when idle" option instead renders every `.tab-dot none` state as `.tab-dot idle`, a quiet grey dot, so this note (and line 52 and test 3) describe the DEFAULT hide-when-idle option, and the grey option is why the shipped re-ink drops both `none` and `idle` before adding the amber `retrying` (a leftover `idle` would dim it; PR 2023). So today the left dot shows working and awaiting only; retrying shows as the amber ring and blocked as the red ring plus fill, neither colouring the dot. The dot is composed as the "before" widget (`render.ts:6368`). The mobile switcher's leading dot maps the same: `#mcur .wd` and `.mrow .workdot` are gold (`--st-working-bg`), `.await` green (`--st-awaitbg-bg`); needs-you shows there as `#mcur.ask`'s dashed magenta border and `.mrow.ask`'s magenta left bar (both in the kernel's mobile CSS strings, cited by selector: the line numbers drift).
