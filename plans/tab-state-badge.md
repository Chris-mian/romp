...
The left STATUS DOT is `.tab-dot`, classed by `tabDotClass`: working is `.tab-dot` (gold, `--st-working-bg`), awaitingBg is `.tab-dot await` (green, `--st-awaitbg-bg`), opening is `.tab-dot opening`, an unknown state is `.tab-dot unknown` (a grey ring), COMPACTING is null (the compacting bar takes the slot, not a dot), and EVERYTHING ELSE, including retrying and blocked, is `.tab-dot none` (hidden); the dot widget's "Grey dot when idle" option instead renders every `.tab-dot none` state as `.tab-dot idle`, a quiet grey dot, so this note (and line 52 and test 3) describe the DEFAULT hide-when-idle option, and the grey option is why the shipped re-ink drops both `none` and `idle` before adding the amber `retrying` (a leftover `idle` would dim it; PR 2023). So today the left dot shows working and awaiting only; retrying shows as the amber ring and blocked as the red ring plus fill, neither colouring the dot. The dot is composed by the before-side compose call in render.ts (the "before" widget). The mobile switcher's leading dot maps the same: `#mcur .wd` and `.mrow .workdot` are gold (`--st-working-bg`), `.await` green (`--st-awaitbg-bg`); needs-you shows there as `#mcur.ask`'s dashed magenta border and `.mrow.ask`'s magenta left bar (both in the kernel's mobile CSS strings, cited by selector: the line numbers drift).
...
Source, in code, so the readings never disagree: the count is `needsYouCount`, an ADDITIVE wire field beside the `needsYou` boolean, computed where the boolean is (the kernel's per-session read in build_session, tallying `_card_needs_you` per sid, the SAME one rule the membership set (needsYou) and the feed's Needs-you COLUMN read). The Needs-you box HEADER counts the ACTIONABLE rows (it drops hard stops and notices) and so can read lower; it is a different tally, not this one. The app-icon badge count skips provisional cards, a third tally. An older page without the field shows a bare dot from the boolean; a newer page shows the count. The digit-ink contrast pin PARSES the sheet (the base rule's color and the `body.theme-light` override, the hex expanded, the light ink resolved by cascade) and asserts 4.5:1 in both themes: black on the dark magenta, `--st-needs-fg` (white) on the light.
...
- An empty dot is 8px at 2px inset; a count makes it a 14px pill (a two-digit count about 16px, "99+" past 99; dense chrome a 12px pill at 1px/1px). The placement and the phone placement approved from the mockups.
