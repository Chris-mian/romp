...
Where the ring shows only the winner, the badge shows EVERY applicable state, one dot each, in a short overlapping row so each reads. The user chose this over a single winning badge (2026-09-21).

Geometry (chosen here; the user corrected the first mockup, 2026-09-21): the dots form a horizontal row along the top, NOT a diagonal fan. Every dot shares the same top inset (2px) and they offset from one another HORIZONTALLY only, each half a dot (4px for an 8px dot) to the LEFT of the one before it, so neighbours overlap by half. The most urgent sits at the corner (rightmost) and on top; the less-urgent dots peek out to its left, each with a drop shadow so the layering reads. Order, most urgent first (rightmost, top of the pile): red (Blocked), then magenta (Needs you), then amber (Retrying). So the first dot sits at `top:2px right:2px`, a second at `top:2px right:6px`, a third at `top:2px right:10px`, each 8px, red highest in z. A three-dot row reaches about `right:18px`, well inside a tab (about 28px tall), so it stays contained.
...
3. Stacking (no precedence, a horizontal row). A tab in more than one state shows one dot PER applicable state, all sharing the same top edge and stepping horizontally by half a dot (equal top insets; right insets stepping by half a dot each), every dot inside the tab's rect, the most urgent at the corner (rightmost) and drawn on top (red over magenta over amber). Assert the dot count equals the applicable-state count, the equal top edges, and the half-dot horizontal offsets. Red at the base: no badge, or a single winner.
