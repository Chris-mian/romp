...
test("the tabOrder frame applies the skeleton list BEFORE applyTabOrder, so its one renderTabs paints the final set", () => {
  ...
  assert.match(RENDER, /else if \(m\.type === "wsup"\) \{ onSocketUp\(skeletonTabs\); skeletonDiagArmed = true; onSocketFlipFrame\(\); \}/,
...
test("requestFullSession(id, why): every ask names its why, from the fixed vocabulary", () => {
  ...
  assert.ok(calls.length >= 13, "the gap ×4, no-base ×2, skeleton-delta ×2, skeleton-click, prefetch, and reattach ×3 (a window at the tail, and a chatTail gap while a window stands MERGES not replaces, T402 round nine)");
  ...
  assert.equal(why("gap"), 4); assert.equal(why("nobase"), 2); assert.equal(why("skeleton-delta"), 2); assert.equal(why("reattach"), 3);   // gap ×4: the index tail's, the uuid tail's, a missing chatHead's and a missing chatMore's (an anchor gone from the transcript); nobase ×2: chatTail and update (statusOnly holds a status for its strip instead)
