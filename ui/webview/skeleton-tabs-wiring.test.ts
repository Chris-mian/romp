...
test("requestFullSession(id, why): every ask names its why, from the fixed vocabulary", () => {
  ...
  assert.ok(calls.length >= 9, "the gap ×5 (three plus the two hunt guards, 2026-09-19), no-base ×2 (chatTail and update; statusOnly holds a status for its strip instead, 2026-09-11), skeleton-delta ×2, skeleton-click and prefetch sites (the reattach site and a missing chatMore's gap retired with the detached client, T386 stage 2)");
  ...
  assert.equal(why("gap"), 5); assert.equal(why("nobase"), 2); assert.equal(why("skeleton-delta"), 2); assert.equal(why("reattach"), 0);   // no detached client, no re-attach (T386 stage 2)   // gap ×5: the index tail's, the uuid tail's, a missing chatHead's, plus upsert's full-frame-desync refusal and regionsAbsorbTail's short-store re-base (the dropped-history guards, 2026-09-19) (an anchor gone from the transcript; chatMore is retired); nobase ×2: chatTail and update (statusOnly holds a status for its strip instead)
