...
test("a create in flight is not draggable, and the page answers the shell's two questions before a move or a close (the chat split, review finds 2026-09-11)", () => {
  ...
  assert.match(RENDER, /tab\.draggable = !s\.sub && !pinned && !fedMissing && !isProvisionalId\(id\) && !settings\.tabsLocked;/);   // …nor a pinned tab (2026-09-10)   // …nor while the tabs are locked (T395)
