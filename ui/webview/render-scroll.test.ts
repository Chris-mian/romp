...
test("a deep-link to an anchor OLDER than the resident tail fetches older history, then lands (the user 2026-06-27)", () => {
  ...
  assert.match(RENDER, /const before = s\.proto === 2 \? s\.firstUuid : s\.headFrom;[\s\S]*?vscodeApi\?\.postMessage\(\{ type: "loadOlder", id: sid, before \}\)/);   // proto 2 anchors by uuid (T323 stage 4b)
