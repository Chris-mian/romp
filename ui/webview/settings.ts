...
export interface RompSettings {
  ...
  tabStateBadge: boolean;   // chat tab strip: THE STATE BADGE (the user 2026-09-21, plans/tab-state-badge.md): on, a tab's state shows as a small round dot at its top-right corner instead of the dashed outline ring, one dot per applicable state in a horizontal top-aligned row, most urgent at the corner. Mutually exclusive with the ring (the ring is suppressed when on); the dashed ring stays the default. Per browser like every gear setting, fanned out by settingsSync. OFF by default; only the literal true turns it on.
}
