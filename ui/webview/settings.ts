...
export interface RompSettings {
  ...
  showArtifactsControl: boolean;   // the Artifacts control (the rail toggle, the phone's tab; plans/artifacts-pane.md, 2026-09-19): shows when on; off, the default, hides it and closes an open pane. A fresh key read like showFilesControl: only the literal true shows it
...
export const DEFAULT_SETTINGS: RompSettings = { tabsLocked: false, compact: true, colormap: "aurora", subgoals: true, showIndexJudges: false, showTriageJudges: false, backend: "sdk", defaultDir: "", showBranch: true, showSessionBadge: false, tabCtx: "over50", stripGroupRows: true, showFilesControl: false, showArtifactsControl: false, chatScheme: "default", chatTabTheme: "classic", theme: "classic", denseChrome: false, paneDocking: false, panes: { timeline: true, fleet: true, feed: true }, statusWidgets: { on: {}, order: [], opts: {} }, tabWidgets: { on: {}, order: [], opts: {} } };
...
export function loadSettings(): RompSettings {
  try {
    ...
    if (raw) {
      ...
      s.showArtifactsControl = s.showArtifactsControl === true;   // the Artifacts control, the same rule (2026-09-19)
