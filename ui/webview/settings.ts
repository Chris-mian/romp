...
export interface RompSettings {
  ...
  showBranch: boolean;       // the MIRROR of the status line's branch widget (T409; the key the toggle used from 2026-06-23 to 2026-09-13): derived from statusWidgets on every load and save, read by no one here; the widget defaults on (the user 2026-09-13). OFF by default (the user 2026-08-10, trimming the statusline for narrow panes; an explicit stored true keeps showing it).
...
export function loadSettings(): RompSettings {
  try {
    ...
    if (raw) {
      ...
      // the status line's widgets (T409): a store from before them derives the branch and the session name from the two
      // keys they replace, showBranch and showSessionBadge, when those are PRESENT (true or false: a reader who chose
      // keeps the choice, and the gear's whole-object saves had merged the old default into every store, which is why
      // the branch's flipped default rides a fresh key), and from the widgets' defaults when absent; a store with them
      // normalizes them and writes both keys back as MIRRORS, so an older reader keeps its meaning
      s.statusWidgets = statusWidgetPrefs("statusWidgets" in parsed ? parsed.statusWidgets : undefined,
                                          { showBranch: parsed.showBranch, showSessionBadge: parsed.showSessionBadge });
