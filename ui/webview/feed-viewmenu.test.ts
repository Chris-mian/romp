...
test("a live menu syncs IN PLACE — rows are never rebuilt under a pressed pointer", () => {
  ...
  assert.ok(paint.includes('const rows = menu.querySelectorAll(".ctx-item:not(.ctx-board)");'), "sync finds the existing four view rows (the Board radios below them paint at build; a pick closes the menu)");
