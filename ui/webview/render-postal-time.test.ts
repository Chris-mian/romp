...
// source level: the in-card postal-time span is gone, and the rail marker no longer excludes postal.
...
test("postal cards no longer render an in-card time", () => {
  assert.doesNotMatch(RENDER, /"postal-time"/, "the in-card postal-time span must be gone");
  assert.doesNotMatch(CSS, /\.postal-time\b/, "the dead .postal-time rule must be removed");
});

test("the rail time-marker is applied to postal turns too (the postal exclusion is gone)", () => {
  assert.doesNotMatch(RENDER, /kind !== "postal"/, "the rail marker must not exclude postal cards");
