...
test("the viewer is READ-ONLY: the message box is hidden, send disabled, one dim statusline line, no meta menus", () => {
  ...
  assert.match(RENDER, /tab\.draggable = !s\.sub && !pinned && !fedMissing && !isProvisionalId\(id\) && !settings\.tabsLocked;/);   // …nor a pinned tab (2026-09-10), nor on a page without its manager (2026-09-10), nor a create in flight (2026-09-11), nor under the tab lock (T395)
