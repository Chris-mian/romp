...
test("executed: the PHONE layout renders the flat strip — every visible id, nothing folded, whatever the store says (sectioning is desktop-only)", () => {
  ...
  const listener = "window.addEventListener(TABGROUPS_EVENT, () => renderTabsAndPrefetch());";
...
test("headers are click-safe: data-act on the node, the action on the stable #tabs delegate, one render path via the event", () => {
  ...
  assert.match(RENDER, /window\.addEventListener\(TABGROUPS_EVENT, \(\) => renderTabsAndPrefetch\(\)\);/, "the same-window delivery");
  assert.match(RENDER, /window\.addEventListener\("storage", \(e\) => \{ if \(e\.key === TABGROUPS_KEY\) renderTabsAndPrefetch\(\); \}\);/, "…and a sibling pane's, through the same helper (round three, medium 1)");
