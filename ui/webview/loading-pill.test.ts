...
test("round five: a reply answers the ask whose key it carries; a clicked-away ask of any kind waits on the wire for its own reply; a stranger is silent", () => {
  ...
  assert.match(fn("chatMore"), /if \(st && st\.proto === 2 && !st\.detached && !msg\.fault && !msg\.missing && msg\.more !== false && Array\.isArray\(msg\.events\) && msg\.events\.length\) \{ landTrail\.push\("more-stranger"\); reattachLive\(msg\.id, true\); \}/, "a stranger newer page that moved the base re-bases the same way, whatever tab is active (round seven, medium 2); more:false already re-attaches, so it is skipped (round eight, low 2)");
