...
test("a click ends the ACTIVE tab's wait and hides the pill whatever the other tabs hold; the in-flight fetch is re-pointed at the reader's own row, a landing's ask files its row as cancelled; a socket flip ends the asks made before it (round two: mediums 1 and 2, lows 1 and 4)", () => {
  ...
  assert.match(win, /if \(st && st\.proto === 2 && !st\.detached && !msg\.fault && !msg\.missing && Array\.isArray\(msg\.events\) && msg\.events\.length\) \{\s*\n\s*landTrail\.push\("window-stranger"\);[^\n]*\n\s*deferReattach\(msg\.id\);/, "a served stranger window at an attached active reader defers the re-base (round six; round nine defers)");
  assert.match(win, /const cancelled = which === "cancelled";/, "…else it is the live or the cancelled ask");
  assert.match(win, /if \(cancelled && msg\.id === activeId\) \{/, "a cancelled reply is handled first and NEVER adopted, whatever the anchor state (T402 round nine, low 3)");
  assert.match(win, /const holdsRow = anchorUuid != null && \(msg\.events \|\| \[\]\)\.some\(\(e: \{ uuid\?: string; key\?: string \}\) => keyOf\(e\) === anchorUuid\);/, "…a detached window that lacks the reader's row triggers a re-land, everything else defers the re-base");
  ...
  assert.match(RENDER, /function onSocketFlipFrame\(\): void \{ if \(!flipPending\) askGen\+\+; flipPending = false; cancelledWire\.clear\(\); baseStale\.clear\(\); cancelOlderWait\("flip"\); \}/, "the reopen FRAME forgets the asks clicked away before the flip and the deferred re-base, and ends the waits asked before it (round three, low 1; round nine)");
...
test("round three: a click's cancel latches at the edge until the reader's own evidence; a fault stands the landing down by the recorded wait; a flip and the pipe's down edge clear and end", () => {
  ...
  assert.match(RENDER, /function onSocketFlipFrame\(\): void \{ if \(!flipPending\) askGen\+\+; flipPending = false; cancelledWire\.clear\(\); baseStale\.clear\(\); cancelOlderWait\("flip"\); \}/, "a flip clears the cancelled set and the deferred re-base (low 1; round nine)");
  assert.match(fn("requestAround"), /noteAsk\(sid, nav, "around", uuid\);/, "a re-land is the page's own ask: no reader wait, no pill (low 2)");
  assert.match(RENDER, /function onPipeDown\(\): void \{ askGen\+\+; flipPending = true; cancelledWire\.clear\(\); baseStale\.clear\(\); cancelOlderWait\("flip"\); \}/, "the VS Code pipe's down edge ends the asks in flight and the deferred re-base (low 3; round nine)");
...
test("round five: a reply answers the ask whose key it carries; a clicked-away ask of any kind waits on the wire for its own reply; a stranger is silent", () => {
  ...
  assert.match(fn("chatMore"), /if \(st && st\.proto === 2 && !st\.detached && !msg\.fault && !msg\.missing && Array\.isArray\(msg\.events\) && msg\.events\.length && !\(msg\.more === false && reachesTail\)\) \{ landTrail\.push\("more-stranger"\); deferReattach\(msg\.id\); \}/, "a stranger newer page that moved the base defers the re-base (round seven medium 2; round eight low 2 skips more:false only when the run reaches the served tail, round nine low 1; round nine defers)");
