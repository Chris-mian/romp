...
test("the pill is for the ACTIVE tab's outstanding reader ask only: not the virtualiser's own re-render, not the walk to the tail, not another tab's wait (T402; round two, medium 1)", () => {
  ...
  assert.match(RENDER, /function noteAsk\(sid: string, reader: boolean, kind: AskKind, key: string \| number \| null \| undefined\): void \{ loadingOlder\.add\(sid\); askedGen\.set\(sid, askGen\); cancelledWire\.delete\(wireKey\(sid, kind, String\(key \?\? ""\)\)\); liveAskKey\.set\(sid, \{ kind, key: String\(key \?\? ""\) \}\); if \(reader\) readerWaits\.set\(sid, askGen\); syncLoadingPill\(\); \}/);
...
test("a click ends the ACTIVE tab's wait and hides the pill whatever the other tabs hold; the in-flight fetch is re-pointed at the reader's own row, a landing's ask files its row as cancelled; a socket flip ends the asks made before it (round two: mediums 1 and 2, lows 1 and 4)", () => {
  ...
  assert.match(win, /if \(st && st\.proto === 2 && !st\.detached && msg\.id === activeId && !msg\.fault && !msg\.missing && Array\.isArray\(msg\.events\) && msg\.events\.length\) \{\s*\n\s*landTrail\.push\("window-stranger"\);\s*\n\s*reattachLive\(msg\.id, true\);/, "a served stranger window at an attached active reader re-bases the kernel, showing nothing (round six, the CI red)");
...
test("round five: a reply answers the ask whose key it carries; a clicked-away ask of any kind waits on the wire for its own reply; a stranger is silent", () => {
  ...
  assert.match(RENDER, /cancelledWire\.delete\(wireKey\(sid, kind, String\(key \?\? ""\)\)\); liveAskKey\.set\(sid,/, "…and a fresh ask supersedes a cancelled twin of its key, so a re-ask whose twin was never answered is not stranded (round six, low 2)");
  ...
  assert.match(fn("chatMore"), /if \(matchAsk\(msg\.id, "newer", typeof msg\.afterUuid === "string" \? msg\.afterUuid : null\) !== "live"\) return;/, "chatMore answers the ask its afterUuid names");
