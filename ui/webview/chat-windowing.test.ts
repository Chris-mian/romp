...
test("the skeleton prefetch never builds a tab the strip does not show (the user 2026-09-14): tabInView and the #only= filter gate it", () => {
  ...
  assert.match(RENDER, /window\.addEventListener\(TABGROUPS_EVENT, \(\) => renderTabsAndPrefetch\(\)\);/, "a section opened re-arms the idle prefetch (round two, medium 1); the executed road is the lab's fold road");
  assert.match(RENDER, /const onOnlyHashChange = \(\): void => renderTabsAndPrefetch\(\);/, "the filter's reveal re-arms the idle prefetch through the one helper (round two of PR 1661, medium 3; PR 1671 round three)");
  assert.match(RENDER, /function renderTabsAndPrefetch\(\): void \{ renderTabs\(\); schedulePrebuild\(\); \}/, "one helper: render, then re-arm");
  assert.match(RENDER, /window\.addEventListener\("storage", \(e\) => \{ if \(e\.key === TABGROUPS_KEY\) renderTabsAndPrefetch\(\); \}\);/, "a sibling document's fold or open re-arms here too (PR 1671 round three, medium 1)");
  assert.match(RENDER, /window\.addEventListener\("storage", \(e\) => \{ if \(e\.key === "romp-chat-cols"\) renderTabsAndPrefetch\(\); \}\);/, "a column's hold change re-arms");
  for (const fn of ["postViews", "postLens", "postTagEdit", "onViewsAck", "onKernelCaps"]) {
    const s = RENDER.indexOf("function " + fn + "("); const e = RENDER.indexOf("\n}\n", s);
    assert.ok(s > 0 && e > s, fn + " exists");
    assert.doesNotMatch(RENDER.slice(s, e), /\brenderTabs\(\);/, fn + " repaints the strip through the helper, never alone (its repaint can reveal tabs)");
  }
});
