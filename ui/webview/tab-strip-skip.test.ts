...
test("every input the strip renders is in the signature", () => {
  ...
  for (const needle of [
    ...
    "tabAskClass(st)", "kst && tabAskClass(kst)",   // the ask ring (2026-09-13): a card of the session's entering or leaving the feed's needs-you column repaints, on a loaded tab and a skeleton alike
  ...
  assert.match(RENDER, /^import \{ tabStateClass, tabAskClass, sectionPip, sectionPipMembers, sectionPipTitle \} from "\.\/tab-state";/m);   // the dot rule moved into the dot widget (T379); + tabAskClass: the ask ring (2026-09-13)
