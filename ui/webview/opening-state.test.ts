...
test("create + connect push the ONE session directly instead of waiting out a full push cycle", () => {
  ...
  assert.ok(KERNEL.includes('push_session=_stage_marked("push.session")(_push_session_now),'), "the backend is wired to it, marked push.session at the hand-off (the backend runs it on a thread of its own, T401 (5a) follow-up)");
