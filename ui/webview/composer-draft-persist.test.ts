...
test("drafts are persisted to and reloaded from the webview's saved state", () => {
  ...
  // citations, attachments, and the doc reader's un-submitted comments (all one draft lifecycle)
  assert.match(RENDER, /function persistDrafts\(\): void \{[\s\S]*setState\?\.\(\{ \.\.\.\(vscodeApi\.getState\?\.\(\) \|\| \{\}\), drafts: Object\.fromEntries\(drafts\),[\s\S]*citations: Object\.fromEntries\(composerCitations\),[\s\S]*files: Object\.fromEntries\(composerFiles\),[\s\S]*docComments: Object\.fromEntries\(docComments\) \}\)/);
