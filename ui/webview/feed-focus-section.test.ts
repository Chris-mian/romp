...
test("the three follow-ups after the review: Tab keeps the copy, Clear from a copy dresses the board's header, a pill repaints both copies", () => {
  ...
  assert.match(FEED, /setSectionChoice\(id, choice === want \? "none" : want\);\s*\n\s*if \(!sectionHosts\(id\)\.length\) applySections\(a, it, distillShown, env\);/); assert.match(FEED, /function reapplyHosts\(id: string\): void \{\s*\n\s*for \(const c of sectionHosts\(id\)\) \{ const h = c as any; if \(h\._it && h\._sectionEnv\) applySections\(h, h\._it, !!h\._distillShown, h\._sectionEnv\); \}/, "the setter re-applies every host of the item (round two of the box content PR)");   // the card's sections, tree and badges moved to card-sections.ts (plans/needs-you.md, one builder with the Needs you row)
