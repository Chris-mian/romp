...
test("the interrupted badge is built once and rides the wrapping chip row", () => {
  ...
  assert.match(FEED, /intBadge\.textContent = BADGE_WORDS\.interrupted\.text;/); assert.match(FEED, /interrupted: \{ text: "interrupted", title: /, "text label, no emoji/glyph (the words in card-sections.ts, worn by the Needs you row too)");
