...
// "stopped on you" over a tab that needed nothing (a false interrupt). Executed on the pure module; the
...
test("executed: sectionPip — a folded header's pip is red ONLY for a member the tab itself renders red", () => {
  ...
  // header pip red, "stopped on you", over an amber tab
  ...
  assert.equal(sectionPip([{ state: "working" }, { state: "needsInput" }]), "blocked", "a card that needs you outranks working");
  ...
  assert.match(SECTION_PIP_TITLE.blocked, /stopped on you/);
...
test("executed: the pip's tooltip names the sessions whose own tab wears its color (the user 2026-09-06)", () => {
  ...
  assert.deepEqual(sectionPipMembers("blocked", members), ["api", "tests"], "a prompt + an on-you API stop; not the auto-retrying one");
  assert.deepEqual(sectionPipMembers("working", members), ["web"]);
  assert.deepEqual(sectionPipMembers("retrying", members), ["old1"]);
  assert.equal(sectionPipTitle("blocked", ["api"]), "a session in this group is stopped on you: api", "one name: the singular phrase");
...
test("executed: the pip's tooltip counts several sessions — never a singular phrase before a list of names", () => {
  // "a session in this group is stopped on you: api, tests" read as one session, then two
  assert.equal(sectionPipTitle("blocked", ["api", "tests"]), "2 sessions in this group are stopped on you: api, tests");
...
// with a card that needs you should grab attention without a click, even while it goes on working in the background.
...
test("executed: tabRingId: the magenta ring for the feed's needs-you verdict in every live state, red over magenta over amber, one ring at a time", () => {
  assert.deepEqual(RING_ORDER, ["ring-needs-you", "ring-waiting-on-you", "ring-retrying"], "the precedence: red, magenta, amber");
  ...
  // the magenta's own test no longer stands down under the red states: the composition's order does
...
test("executed: tabRingId under the switches — a ring switched off hands the tab to the next ring that applies, or to nothing", () => {
  assert.equal(tabRingId({ state: "needsInput", needsYou: true }, off("ring-needs-you")), "ring-waiting-on-you", "red off: a live prompt with a card is magenta, which is true of that tab");
  assert.equal(tabRingId({ state: "needsInput" }, off("ring-needs-you")), null, "red off, no card: a plain tab");
  assert.equal(tabRingId({ state: "working", needsYou: true }, off("ring-waiting-on-you")), null, "magenta off: a working card is nothing");
  assert.equal(tabRingId({ state: "retrying", needsYou: true }, off("ring-waiting-on-you")), "ring-retrying", "magenta off: the amber shows through");
...
test("executed: sectionPip: a hidden member's card that needs you is magenta, between red and gold, so a fold never hides it; under the switches the pip follows the members' tabs", () => {
  ...
  assert.equal(sectionPip([{ state: "needsInput", needsYou: true }, { state: "ready" }], off("ring-needs-you")), "ask", "the red ring switched off: a hidden member with a prompt and a card shows the fold's pip magenta");
  ...
  assert.equal(sectionPip([{ state: "working", needsYou: true }], off("ring-waiting-on-you")), "working", "magenta off: the working dot is the state's, not a ring's, and stays");
  assert.equal(sectionPip([{ state: "working" }], off("ring-needs-you", "ring-waiting-on-you", "ring-retrying")), "working");
  assert.match(SECTION_PIP_TITLE.ask, /a card that needs you/);
  assert.equal(SECTION_PIP_TITLE_MANY.ask(2), "2 sessions in this group have a card that needs you");
  assert.doesNotMatch(SECTION_PIP_TITLE_MANY.ask(2), /\ba session\b| is | has /, "the counted phrase is plural throughout");
  assert.equal(sectionPipTitle("ask", ["api"]), "a session in this group has a card that needs you: api");
  assert.equal(sectionPipTitle("ask", ["api", "web"]), "2 sessions in this group have a card that needs you: api, web");
});
...
test("executed: the pip's tooltip names the members by the ring their tab wears under the switches — a working session with an ask is named under both kinds", () => {
  ...
  assert.deepEqual(sectionPipMembers("ask", members, off("ring-needs-you")), ["web", "api", "tests"], "the red ring switched off: the prompt's tab wears the magenta, so the magenta pip names it");
