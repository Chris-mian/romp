...
test("the Tab widgets section's rows come from the strip's own module: built once, painted in place, a sliding switch, house pickers for the options", () => {
  ...
  assert.ok(chatPane.indexOf("id=rs-widgets") < chatPane.indexOf("With the outline ring, one cue at a time: the first state that applies wins, in this order. With the state badge on, the Needs-you count also shows beside Blocked and retrying.") && chatPane.indexOf("With the outline ring, one cue at a time: the first state that applies wins, in this order. With the state badge on, the Needs-you count also shows beside Blocked and retrying.") < chatPane.indexOf("id=rs-rings") && chatPane.indexOf("id=rs-rings") < chatPane.indexOf("data-section=statusline"), "the hint and the rings' host follow the title rows, before the Status line section");
