...
test("render.ts: a pinned tab is not draggable, wears the pushpin after its hot key, and is in the strip's signature", () => {
  ...
  assert.match(RENDER, /tabHotkey\(id\), pins\.has\(id\)\]/, "an input the strip paints is in its signature");
  ...
  assert.match(RENDER, /tab\.draggable = !s\.sub && !pinned && !fedMissing && !isProvisionalId\(id\) && !settings\.tabsLocked;/, "the pinned tab does not start a drag (nor any tab on a page without its manager, a create in flight, or under the tab lock — main, 2026-09-13)");
});
...
test("a tab menu taller than the window scrolls inside it, and its own scroll does not dismiss it (CI find 2026-09-13: with one more row the Billing row sat below a 300px window's edge)", () => {
  ...
  assert.match(RENDER, /window\.addEventListener\("scroll", \(e\) => \{ if \(ctxMenuEl && ctxMenuEl\.contains\(e\.target as Node\)\) return; dismissTabMenu\(\); \}, true\);/, "a scroll inside the menu is not a dismissal; the page's still is");
