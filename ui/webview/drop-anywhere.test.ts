...
test("kernel: every non-chat pane's shim and the shell document refuse a file drag — not-allowed cursor, drop swallowed, no navigation", () => {
  const shim = KERNEL.slice(KERNEL.indexOf("def _shim(app, v=0, no_stale=False, pv=None, data=None):"), KERNEL.indexOf("def _shim(app, v=0, no_stale=False, pv=None, data=None):") + 40000);
