...
test("courier handoff: the '↪ from <sender>' origin marker is wired and styled", () => {
  ...
  assert.match(FEED, /row2\.append\(idwrap, origin, reBadge, fupBadge, nfBadge, warnChip, waitOnBadge\)/, "the origin marker rides the name row beside the chips");
...
test("the follow-up badge serves ONLY '↩ re-judging' now — the '↻ Followed up' chip was removed (the user 2026-07-01)", () => {
  ...
  assert.match(FEED, /row2\.append\(idwrap, origin, reBadge, fupBadge, nfBadge, warnChip, waitOnBadge\)/);
