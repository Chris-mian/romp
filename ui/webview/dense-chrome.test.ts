...
test("every dense rule is scoped to the body class, and the sheet's defaults are untouched", () => {
  ...
  for (const sel of [".tab", ".tab-add", ".tab-tagbox", ".tab-strip-end", ".tab-widgets-gear", ".tab-group-head", ".tab-group-sep:not(.tab-group-break)", "#bg-tasks", ".bg-fold-head", ".bg-list", ".bg-group-head", ".bg-head", ".bg-sum", ".bg-since"]) {
