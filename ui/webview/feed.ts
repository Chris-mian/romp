...
function updateAskCard(card: HTMLElement, it: AskItem) {
  ...
  if (it.provisional) {
    ...
    // outline the card in ITS session's identity colour at 0.7 alpha (the user 2026-07-15 — softer than solid
    ...
    card.style.borderColor = it.color ? `color-mix(in srgb, ${it.color.bg} 70%, transparent)` : `rgba(${r}, ${g}, ${b}, ${Math.min(TINT_ALPHA + 0.2, 0.9)})`;
  }
...
function updateGroupCard(card: HTMLElement, g: AskGroup) {
  ...
  // outline in the group's session identity colour at 0.7 alpha (the user 2026-07-15), recency tint fallback
  card.style.borderColor = g.color ? `color-mix(in srgb, ${g.color.bg} 70%, transparent)` : `rgba(${r}, ${gg}, ${b}, ${Math.min(TINT_ALPHA + 0.2, 0.9)})`;
