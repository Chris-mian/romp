...
test("the filter defaults to NOTHING selected and only ever narrows the RENDER, never the data", () => {
  ...
  assert.ok(FEED.includes("let shown = viewFiltered(asks).filter((a) => boardOf(a) === board);"), "render reads the shared view (then keeps the active board's cards, phase four)");
