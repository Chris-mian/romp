...
test("kernel: the clears-log refusal names the request the way _refuse_drive's frame does", () => {
  ...
  assert.match(FEED, /function setUndoFloor\(host: string, floor: unknown\): void \{\n\s+if \(typeof floor !== "number"\) return;\n\s+for \(const c of restoreChecks\.values\(\)\) if \(c\.host === host\) c\.floor = floor;/, "an account's floor lands on its own kernel's checks alone");
  assert.match(FEED, /if \(op === "undoClear"\) setUndoFloor\(fromHost, m\.buildId\);/, "the refusal's and the reorder's frames carry it");
  ...
  assert.match(KERNEL, /client\["send"\]\(json\.dumps\(\{"type": "undoAck", "op": op or "", "buildId": _floor\}\)\)/, "and the ack for a landed undo");
