...
test("kernel: the clears-log refusal names the request the way _refuse_drive's frame does", () => {
  ...
  assert.match(KERNEL, /if key != LEDGER_READ_KEY:[\s\S]{0,900}if not _cleared_read_fault\[0\]:\n\s+frame\["batches"\], frame\["owedBatch"\], frame\["batchesTotal"\] = _lb\[0\]/, "every account carries the kernel's stack and the count before the bound, except while the log cannot be read: the read-fault account and any account beside it ship none (the round-one verifier of PR 2025)");
  ...
  assert.match(FEED, /\} else if \(m\.type === "hostUp" && Array\.isArray\(m\.hosts\)\) \{[\s\S]{0,600}for \(const h of m\.hosts\) if \(typeof h === "string" && h\) dropChecksOnReconnect\(h\);/, "the tunnel poll's word as a second trigger");
