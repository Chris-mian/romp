...
test("kernel: the clears-log refusal names the request the way _refuse_drive's frame does", () => {
  ...
  assert.match(KERNEL, /except FileNotFoundError:\n\s+_CLEARED_STATS\["derived"\] \+= 1\n\s+_cleared_read_fault\[0\] = ""[^\n]*\n\s+return cur\n\s+except \(OSError, ValueError\) as e:[\s\S]{0,900}if _cleared_read_fault\[0\] != copy:\n\s+_cleared_read_fault\[0\] = copy\n\s+_clears_log_fault_note\("the read", e\)/, "an absent clears log stays silent; a present one that cannot be read, or is not text, reads as the empty uncached set and is SAID once per episode (the post-merge review of PR 2021)");
  assert.match(KERNEL, /skipped\[LEDGER_READ_KEY\] = \{"fault": _cleared_read_fault\[0\], "ids": \[\]\}/, "and the undo's account names it");
