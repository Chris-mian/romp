...
def _deferral_at(rec):
    """When a `deferred` record started. Records were a bare epoch int before the why was stored
    alongside it (2026-07-23); a live state file still holds those, so read both shapes."""
    if isinstance(rec, dict):
        try:
            return int(rec.get("at") or 0) or None
        except (TypeError, ValueError):
            return None
    try:
        return int(rec) or None
    except (TypeError, ValueError):
        return None


def _deferral_why(rec):
...
def _stalled_goals():
    """{gid: {"why", "since"}} for every goal the nudge gate is HOLDING on a reviver that isn't retiring —
...
def _drive(msg, client):
    ...
    # for the rest of REBUILD_MIN_S, and the card took ~a second to fly to Working (the user 2026-07-16).
    # Every other drive op already wakes the pusher; the ask family was the one that answered into silence.
...
def _seg_placed(placements, seg_id):
    ...
    (value None) still counts as placed — the planner ruled; nothing is pending. Every segment (text-less
    seams included) now carries a UNIQUE _seg_key — a text-less seam is keyed by its anchor atom's uuid, not
    the shared sha1('') (em._segment_id, the user 2026-07-22) — so the fuzzy match aliases nothing and needs
    no t-window."""
...
def _seg_work_caption(caps, seg_id):
    ...
    grain-'segment' rows only (turn rows carry a different key). Text-less seams now carry a UNIQUE
...
_JUDGE_FAMILY = {"gister": "captioner", "opener": "planner", "prompt-planner": "planner",
                 "placer": "planner", "briefer": "distiller", "staller": "distiller",
                 "consolidator": "grouper"}
