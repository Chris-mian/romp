...
!!! warning "The VS Code port forwarder is not a good path for the browser dashboard"
    ...
    deltas instead of whole payloads, and every frame over a kilobyte is
    compressed for a client that offers WebSocket compression
    (`permessage-deflate`, which every browser and Node's `ws` do; a whole
    feed frame that measured 2.5 MB plain crosses as about 0.4 MB). That
    keeps a forwarded dashboard usable,
