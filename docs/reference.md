...
rearranged reads in that order while a phone, which has no drag, reads them in the order the sessions were
started. The groups, their order, and which sessions sit in each of them are the same everywhere. The Sessions pane has the same
