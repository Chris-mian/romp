...
(`POST /deliver`): the kernel enqueues the banner on the session's SDK input
queue — first-class, nothing to scrape — with the Stop-hook drain
(`hooks/romp-postal-drain.sh`) as the turn-boundary backstop. Non-delivery is
...
so the TOC is free. (Caveat: a live AskUserQuestion/ExitPlanMode is in the tree as
an unanswered tool_use until it resolves. The chip state comes from `states/`
regardless.)
